...
bool series_guessing_game();
bool number_ordering_game();
// =========================== //
...
int main() {
  ...
  do {
    ...
    std::cout << "[4] Series Guessing Game" << std::endl;
    std::cout << "[5] Number Ordering Game" << std::endl;
    ...
    switch ( menu_selector ) {
      ...
      case 4:
        result = series_guessing_game();
        games++;
        break;

      case 5:
        result = number_ordering_game();
        games++;
        break;
...
bool series_guessing_game() {
  ...
  // Generate Two Random Ints in the Range (1-10). //
  
  srand( time( NULL ) );
  int starting_number = ( rand() % 10 ) + 1;
  int common_difference = ( rand() % 10 ) + 1;
  
  // ============================================  //

  std::cout << std::endl << "What is the common difference of the given arithmetic series?" << std::endl;
  std::cout << "Series: | ";

  for (int i=0; i <= 9; i++) {
    std::cout << starting_number + ( common_difference * i ) << " | "; 
  }

  std::cout << std::endl << "What is the common difference: "; 
  std::cin  >> guess;

  if (guess == common_difference) {
    std::cout << "Congratulations, you are correct!" << std::endl;
    return true;
  }
...
bool number_ordering_game() {
  ...
  // Generate Two Random Ints in Ranges (1-50) and (1-10) //
  ...
  std::cout << std::endl << "Put the numbers " << random[0] << ", " << random[1] << ", " << random[2] << ", and " \
  << random[3] << " in order from least to greatest. [1-4]" << std::endl;
  std::cout << "(If two or more numbers are equal they can be placed in any order.)" << std::endl;

  for (int i=0; i <= 3; i++) {
    std::cout << "Where in order is " << random[i] << ": ";
    ...
    do {
      ...
      if ( (index < 1) || (index > 4) ) { 
        std::cout << "Your guess was out of range [1-4], try again: ";
      }

      else if ( order[index - 1] != 0 ) {
        std::cout << "You already placed a number there. No take backs! Try again: ";
      }

      else {
        order[index - 1] = random[i];
        break;
      }
  ...
  for (int i=0; i <= 2; i++) {
    if ( !( order[i] < order[i + 1]) ) {
      std::cout << std::endl << "Sorry! " << order[i + 1] << " comes before "  << order[i] << "." << std::endl;
      return false;
    }
  }

  std::cout << "Congratulations! You put the numbers in the correct order!" << std::endl;
  return true;
